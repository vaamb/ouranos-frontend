<script>
  ...
  import {
    faAddressCard, faUserCog, faPowerOff
...
<nav style="--menu-width:{ width }">
  <div class="top-box" tabindex="0" role="button" aria-pressed="false" on:click={ toggleMenu }>
    ...
    <div class="user-box">
      <img
        src="/images/avatar/{ $currentUser.avatar }_64.jpg"
        alt="User avatar"
        class="avatar"
      />
      <div class="welcome">
  ...
  <div class="toggle-accordion" class:show="{ showMenu === true }">
    <ul class="accordion">
      {#each layout.items as item, index }
        <MenuItem item={ item } open={ openSubMenuIndex === index } on:click={ () => toggleSubMenu(index) } />
      {/each}
    </ul>
