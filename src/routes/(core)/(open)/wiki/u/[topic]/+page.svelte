<script>
	...
	import { capitalize, slugify } from "$lib/utils/functions.js";
...
<Table
	...
	columns={[
		...
		{ key: 'tags', label: 'Tags', serializer: joinTags },
		{
			...
			label: 'Link',
			isLink: true,
			serializer: (value) => `/wiki/u/${slugify(topicName)}/u/${slugify(value)}`
		}
	]}
	data={wikiArticles}
	editable={true}
	...
	<Form
		...
		on:confirm={(event) => {
			...
			.then((contentText) => {
				...
				crudRequest(`app/services/wiki/topics/u/${topicName}/u`, 'create', payload)
				.then(() => {
					fetchWikiArticles(topicName).then((data) => {
						wikiArticles = data;
...
{#if wikiArticles[crudIndex]}
	<Modal
		...
		on:confirm={() => {
			crudRequest(
				`app/services/wiki/topics/u/${topicName}/u/${wikiArticles[crudIndex]['name']}`,
				'delete'
			).then(() => {
				fetchWikiArticles(topicName).then((data) => {
					wikiArticles = data;
