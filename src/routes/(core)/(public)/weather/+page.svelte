<script>
	...
	// Shared with the theme, which needs the same sun times to run its `auto`
	// mode, so whichever gets there first pays for the request.
	let suntimes = $derived(servicesState.suntimes);

	// Chart.js resolves no CSS variables, so the chart's colours have to be read
	// off the document as literals — and re-read on every theme flip. Reading them
	// once was a real bug: a chart first drawn at night kept the dark theme's pale
	// `--text` and turned into a white line on the day theme's white card.
	//
	...
	// everything else is clamped there, or a calm night pulls the wind axis into
	// negative kilometres per hour.
	const measures = {
	...
	// One series at a time, so one ink and no legend — the title names it. A hue
	// per measure would have diluted the rule that colour is semantic here.
	...
	// indexed together. The old page skipped index 0 because the block above it
	// already covered today; here today is the first row of the week.
...
{#if !isEmpty(currently)}
	...
	<section class="now-card">
		...
		<!-- Sunrise and sunset are a different kind of fact from the readings
		     above — astronomy, not a measurement — so they get their own block
		     under an eyebrow rather than another metric cell. -->
	...
	<section class="chart-card">
		...
		<div class="plot-wrap">
			<!-- `Graph` builds its scales once and never revisits them, so anything
			     ...
			     range) and the theme (which sets the grid and tick colours). Keyed on
			     the resolved colour rather than the theme itself, because the colours
			     are read a frame later — keying on the theme would remount with the
			     outgoing palette. -->
